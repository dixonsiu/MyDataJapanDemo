...
addLoadScript = function (scriptList) {
    scriptList.push("./js/jdenticon@1.8.0");
    return scriptList;
}
    ...
    ut.loadScript(function () {
        ...
        if (mode === "demo") {
            ...
            if (_.isUndefined(target) || _.isNull(target)) {
                targetCellName = "dixonsiu";
            } else {
                targetCellName = target[1] || "dixonsiu";
            }
            sessionStorage.setItem("cellname", targetCellName);
            sessionStorage.setItem("isDemo", "true");
            sessionStorage.setItem("planStatus", JSON.stringify(planStatus[targetCellName]));
            sessionStorage.setItem("recommendedSchedule", JSON.stringify(recommendedSchedule[targetCellName]));
            lg.rootUrl = "../data/" + targetCellName + "/";
            if (cellProfile[targetCellName]) {
                $.extend(tempProfile, cellProfile[targetCellName].default, cellProfile[targetCellName][i18next.language]);
            }
            lg.populateProfile(tempProfile);
            lg.setBizTheme();
            lg.automaticLogin();
...
lg.populateProfile = function(tempProfile) {
    let profile = _.clone(tempProfile);
...
lg.sendAccountNamePw = function(username, pw) {
    ...
    $.ajax({
        ...
        let isDemo = JSON.parse(sessionStorage.isDemo || false);
        if (isDemo) {
            location.href = "./main.html";
        } else {
            // login failed
